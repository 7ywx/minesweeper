    ...
    def start(self):
        ...
        while flag:
            ...
            if self.area[row - 1][col - 1] == 'X':
                ...
                # 上方元素（如果存在）
                if row > 0:
                    grid[row - 2][col - 1] = self.area[row - 2][col]

                # 下方元素（如果存在）
                if row < 8:
                    grid[row][col - 1] = self.area[row + 1][col]

                # 左侧元素（如果存在）
                if col > 0:
                    grid[row - 1][col - 2] = self.area[row - 1][col - 1]

                # 右侧元素（如果存在）
                if col < 8:
                    grid[row - 1][col] = self.area[row - 1][col + 1]

                # 对角线方向的元素（如果存在）
                if row > 0 and col > 0:
                    grid[row - 2][col - 2] = self.area[row - 2][col - 1]

                if row > 0 and col < 8:
                    grid[row - 2][col] = self.area[row - 2][col + 1]

                if row < 8 and col > 0:
                    grid[row][col - 2] = self.area[row + 1][col - 1]

                if row < 8 and col < 8:
                    grid[row][col] = self.area[row + 1][col + 1]
                self.print_grid_with_coordinates(self.area)
                self.print_grid_with_coordinates(grid)
            else:
                grid[row - 1][col - 1] = self.area[row - 1][col - 1]
                self.print_grid_with_coordinates(grid)
            # flag = False
game = Game()
game.start()
