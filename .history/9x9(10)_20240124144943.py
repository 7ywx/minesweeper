    ...
    def start(self):
        ...
        while flag:
            ...
            if self.area[row - 1][col - 1] == 'X':
                ...
                # 上方元素（如果存在）
                if i > 0:
                    grid[row - 1 - 1][col - 1] = self.area[i - 1][j]
                ...
                # 左侧元素（如果存在）
                if j > 0:
                    grid[row - 1][col - 2] = self.area[i][j - 1]

                # 右侧元素（如果存在）
                if j < cols - 1:
                    grid[row - 1][col] = self.area[i][j + 1]

                # 对角线方向的元素（如果存在）
                if i > 0 and j > 0:
                    grid[row - 2][col - 2] = self.area[i - 1][j - 1]

                if i > 0 and j < cols - 1:
                    grid[row - 2][col] = self.area[i - 1][j + 1]

                if i < rows - 1 and j > 0:
                    grid[row][col - 2] = self.area[i + 1][j - 1]

                if i < rows - 1 and j < cols - 1:
                    grid[row][col] = self.area[i + 1][j + 1]
                self.print_grid_with_coordinates(grid)
            else:
